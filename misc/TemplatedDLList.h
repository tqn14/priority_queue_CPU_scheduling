...
void DLList<T>::insert(DLList<T> &dll, T obj) { 
  ...
  {
    if (dll.first_node()->getObj() > node->getObj())
    {   
      insert_before(*(dll.first_node()), obj);
    } 
    else if ((dll.after_last_node()->prev)->getObj() <= node->getObj()) 
    {
      ...
      while (((curr->next) != &trailer) && ((curr->next)->getObj() <= node->getObj())) { 
          curr = curr ->next; 
      }
      if (node->getObj() < curr->getObj()) {
